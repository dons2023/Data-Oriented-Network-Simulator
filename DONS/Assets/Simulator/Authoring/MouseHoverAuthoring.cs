...
using UnityEngine;
using static PhysicsSamplesExtensions;
...
public partial class MouseHoverSystem : SystemBase
{
    ...
    public struct WorldRaycastJob : IJob
    {
        ...
        public NativeReference<Unity.Physics.RaycastHit> RaycastHitRef;

        public void Execute()
        {
            var mousePickCollector = new MousePickCollector(1.0f, CollisionWorld.Bodies, CollisionWorld.NumDynamicBodies);
            //Debug.Log($"CollisionWorld.Bodies.Count:{CollisionWorld.Bodies.Length}");
    ...
    {
        if (MouseControl.Instance.IsEnabledMouseHover)
        {
            var collisionWorld = m_BuildPhysicsWorld.PhysicsWorld.CollisionWorld;
            Vector2 mousePosition = Input.mousePosition;
            UnityEngine.Ray unityRay = CaremaChange.Instance.CurrentCarema.ScreenPointToRay(mousePosition);
            ...
            if (!mouseHover.CurrentEntity.Equals(graphicsEntity))
            {
                ...
                if (hasPreviousEntity && EntityManager.HasComponent<RenderMesh>(mouseHover.PreviousEntity))
                {
                    ...
                    //Debug.Log($"0,mouseHover.PreviousEntity:{mouseHover.PreviousEntity.Index},mouseHover.CurrentEntity:{mouseHover.CurrentEntity.Index}");
                }
                ...
                if (hasCurrentEntity && EntityManager.HasComponent<RenderMesh>(mouseHover.CurrentEntity))
                {
                    ...
                    //Debug.Log($"1,mouseHover.PreviousEntity:{mouseHover.PreviousEntity.Index},mouseHover.CurrentEntity:{mouseHover.CurrentEntity.Index}");
                }
            ...
            if (graphicsEntity != Entity.Null && Input.GetMouseButtonDown(0) && MouseControl.Instance.IsEnabledMouseClick && !isMouseClicked)
            {
                isMouseClicked = true;
                Debug.Log("Click Line!");
                if (EntityManager.HasComponent<Line_In_Out_PortData>(graphicsEntity))
                {
                    Debug.Log("HasComponent!");
                    Line_In_Out_PortData line_In_Out_PortData = GetComponent<Line_In_Out_PortData>(graphicsEntity);
                    var e = EntityManager.CreateEntity();
                    EntityManager.AddComponentData(e, new LinkCongestioneFlag()
                    {
                        line_In_Out_PortData = new Line_In_Out_PortData()
                        {
                            InID = line_In_Out_PortData.InID,
                            OutID = line_In_Out_PortData.OutID
                        }
                    }); ;
                }
            }
